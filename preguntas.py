...
def pregunta_09():
    ...
    """
    dg = tbl0.copy()
    dg['year'] = dg['_c3'].str[:4].astype(str)
    return dg
...
def pregunta_10():
    ...
    """
    dg = tbl0.copy()
    result = dg.groupby('_c1')['_c2'].agg(lambda x: ':'.join(map(str, sorted(x)))).reset_index()
...
def pregunta_12():
    ...
    df = tbl2.groupby('_c0')['_c5'].apply(lambda x: ','.join(sorted(x))).reset_index()
...
def pregunta_13():
    ...
    merged_data = pd.merge(tbl0, tbl2, on='_c0')
    sum_data = merged_data.groupby('_c1')['_c5b'].sum()
    return sum_data
